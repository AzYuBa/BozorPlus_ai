  ...
  return (
    <div className="text-slate-900">
      ...
      <div className="flex gap-2 mt-3">
        <input type="number" className="border rounded-lg px-3 py-2" value={turnover} onChange={(e) => setTurnover(Number(e.target.value))} />
        <input type="number" className="border rounded-lg px-3 py-2" value={expenses} onChange={(e) => setExpenses(Number(e.target.value))} />
        <button className="bg-violet-600 text-white px-4 rounded-lg" onClick={run}>
          Taqqoslash
        </button>
      </div>
      {tax && (
        <div className="mt-4 bg-white border rounded-2xl p-4 h-72">
          <ResponsiveContainer>
      ...
      <ul className="mt-3 text-sm space-y-2">
        {(tax?.rows || []).map((r: any) => (
          <li key={r.regime} className="bg-white border rounded-xl p-3">
            <b>{r.regime}</b> — {som(r.tax)} · stavka {r.rate}%
            <div className="text-xs">
              <a className="text-violet-600" href={r.legal_ref_url} target="_blank">
                lex.uz
              </a>{" "}
      ...
      {st && (
        <div className="mt-4 grid md:grid-cols-3 gap-3">
          {st.variants.map((v: any) => (
            <div key={v.status} className="bg-white border rounded-2xl p-4">
              <div className="font-medium">{v.label}</div>
              <div className="text-sm">Soliq {som(v.tax)}</div>
