      ...
      <div className="grid lg:grid-cols-2 gap-4 mt-4">
        <div className="bg-panel/60 border border-line rounded-2xl p-4 space-y-2">
          ...
          {["amount", "rate", "months", "grace", "subsidy"].map((k) => (
            <label key={k} className="block text-sm text-ink/80">
              {k}
              <input
                type="number"
          ...
          <button className="bg-accent px-4 py-2 rounded-lg" onClick={calc}>
            Hisoblash
          </button>
          {loan && (
        ...
        <div className="bg-panel/60 border border-line rounded-2xl p-4">
          ...
          <p className="text-xs text-muted mt-2">{kti?.disclaimer}</p>
        </div>
      </div>
